...
<template>
  <section>
    <header class="page-header">
      <div>
        <h2>Signals</h2>
        <p>Shortage, overstock, and expiration-risk recommendations with trace IDs.</p>
      </div>
    </header>
    <div class="panel table-panel">
      <table class="data-table">
        <caption>
          Traceable signal queue
        </caption>
        <thead>
          <tr>
        ...
        <tbody>
          <tr v-for="row in signalRows" :key="row.id">
            ...
            <td>{{ row.type }}</td>
            <td>
              <span class="status-chip" :data-tone="row.severity">{{ row.severity }}</span>
            </td>
            <td>{{ row.site }}</td>
          </tr>
        </tbody>
