...
export function claimOutboxBatch(
  ...
  const availableRecords = records
    .filter(
      (record) =>
        record.status === "pending" && !record.lockedAt && dueAt(record) <= now,
    )
    .sort((left, right) => {
  ...
  const claimed = availableRecords
    .slice(0, Math.max(1, batchSize))
    .map((record) => lockOutboxRecord(record, workerId, now));
...
export async function processOutboxBatch(
  ...
  const recordsById = new Map(
    claimResult.records.map((record) => [record.id, record]),
  );
...
export function acknowledgeOutboxRecord(
...
export function failOutboxRecord(
  ...
  const nextAvailableAt = retryable
    ? delayAt(now, retryDelayMs(attemptCount))
    : now;
  const timestamp = now.toISOString();
